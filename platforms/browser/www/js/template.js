		...
		t+='<ons-list-item tappable modifier="chevron" onclick="loadItem(' + "'" + val.cat_id + "'," + "'" + addslashes(val.category_name) + "'" + '   )">';
	      t+='<ons-row>';
	           ...
	           t+='<ons-col width="100px" >';
	             t+='<div class="pending_el" style="height:70px;width: 70px;color: #ccc;border-radius:50%;" ></div>';
	             t+='<img class="rounded_image" src="'+val.photo_url+'">';
	           t+='</ons-col>';
...
var displayItem = function(data , cat_id) {
	...
	$.each( data, function( key, val ) {
		...
		t+='<ons-list-item tappable onclick="itemDetails(' + "'" + val.item_id + "', " + "'" + cat_id +  "'" +  ')">';
	      t+='<ons-row>';
	           	           
	           t+='<ons-col>';
	             t+=val.item_name;
	             t+='<p class="small ">'+val.item_description+'</p>';
		             ...
		             if($.isArray(val.prices)) {	             	
		             	$.each( val.prices, function( price_key, price_val ) { 
		             		  ...
		             		  if (price_val.discount_price>0.0001){
		             		  	  t+= '<div class="price">'+ size + '<span class="tag_discount">'+price_val.formatted_price+'</span>' + price_val.formatted_discount_price   +'</div>'
		             		  } else {
		             		  	  t+= '<div class="price">'+ size + '<span class="spacer">' + price_val.formatted_price +  '</span>' +  '</div>'
		             		  }		             		  
		             	});
		             }
	             }
	...
	if ( datas.transaction_type == "delivery" ){
		...
		html+='<ons-list-item tappable modifier="chevron longdivider" onclick="initAddress()" >';	
		  html+='<div class="left">' + t("Delivery Address")  +'</div>';
		   html+='<div class="right"><ons-icon icon="md-home" size="20px" class="color_green"></ons-icon> <span class="list-item__subtitle delivery_address_label concat-text">'+ selected_delivery_address +'</span></div>';
		html+='</ons-list-item>';
	}
...
var addressList = function(data){
	...
	html+='<ons-list modifier="list_menu">';
	$.each( data, function( key, val ) {
		html+='<ons-list-item modifier="longdivider" tappable onclick="cardsAction(' + "'" + val.id + "'," + "'" + "address" + "'" + ' )"  >';
        html+='<div class="left concat_text concat_text_min">'+val.address+'</div>';
         if (val.as_default==2){
...
addButtonReview = function(enabled){
	...
	t+='<ons-fab position="bottom right" onclick="showPage(\'add_review.html\')">';     
       t+='<ons-icon icon="ion-android-add" class="color_green"></ons-icon>';
    t+='</ons-fab>';
